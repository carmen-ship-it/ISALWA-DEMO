...
describe('production workspace page', () => {
  it('uses the Spanish step names and does not treat a pedido as the parent of a quema', () => {
    assert.match(page, /ProductionPostSaleDesk/);
    assert.match(surface, /PRODUCTION_STEP_LABELS/);
    ...
    assert.match(workspace, /PRODUCTION_PAGE_COPY\.quemaNotParent/);
    assert.match(workspace, /PRODUCTION_PAGE_COPY\.receiptDoesNotAssign/);
