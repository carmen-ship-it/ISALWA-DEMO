...
import { dataHealthFromSummaries, mapCoverage } from '@/lib/party/data-health';
  ...
  return (
    <PageContainer label="Mapa">
      <PageHeader
        kicker="Ubicación"
        title="Mapa"
        description="Solo coordenadas ya registradas. Un enlace de Maps no coloca al cliente en el mapa."
        action={
          ...
          <ul className="mt-3 space-y-4">
            {issues.map((issue) => (
              <li key={issue.id} className="border-t border-[var(--isalwa-mist)] pt-4">
                <StatusPill tone={issue.id.startsWith('shared-provenance') ? 'manual' : 'warning'}>
                  {issue.title}
                </StatusPill>
