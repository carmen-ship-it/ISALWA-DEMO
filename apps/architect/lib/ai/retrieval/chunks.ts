/**
 * Chunk retrieval — thin on purpose.
 *
 ...
 * introduce a second retrieval system — `pack.ts` composes this for the
 * document-chunk tier of `RetrievalPack` instead of duplicating it.
 *
 * Moved out of `index.ts` (Mission C) only so `pack.ts` can import it
 * without a circular dependency on the barrel that re-exports both.
 */
